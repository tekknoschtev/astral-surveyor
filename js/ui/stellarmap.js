...
export class StellarMap {
    ...
    handleStarSelection(mouseX, mouseY, discoveredStars, canvas, discoveredPlanets) {
        if (!discoveredStars)
            return false;
        ...
        if (this.zoomLevel > 3.0 && discoveredPlanets) {
            for (const planet of discoveredPlanets) {
                ...
                if (planet.x === null || planet.y === null)
                    continue;
    ...
    render(renderer, camera, discoveredStars, gameStartingPosition, discoveredPlanets) {
        if (!this.visible)
            return;
    ...
    renderOrbitalCircles(ctx, starMapX, starMapY, planets, scale, mapX, mapY, mapWidth, mapHeight) {
        ...
        for (const planet of planets) {
            ...
            const mapOrbitalRadius = Math.sqrt(Math.pow(planetMapX - starMapX, 2) +
    ...
    renderPlanetInfoPanel(ctx, canvas) {
        if (!this.selectedPlanet || !this.namingService)
            return;
        // Panel dimensions and position
        const panelWidth = 300;
        const panelHeight = 120;
...
//# sourceMappingURL=stellarmap.js.map