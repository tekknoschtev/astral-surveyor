...
export class TouchUI {
        ...
        for (const button of this.buttons) {
            if (button.alpha > 0.1) { // Only check visible buttons
                ...
                if (button.id === 'followShip' || button.id === 'zoomIn' || button.id === 'zoomOut' || this.isTouchDevice) {
                    const distance = Math.sqrt(Math.pow(touchX - button.x, 2) +
    ...
    renderBrakeFeedback(renderer, input) {
        if (!this.isTouchDevice)
            return;
    ...
    hideMapControls() {
        ...
        setTimeout(() => {
            this.buttons = this.buttons.filter(b => b.id === 'mapToggle' || b.alpha > 0.01);
...
//# sourceMappingURL=touchui.js.map