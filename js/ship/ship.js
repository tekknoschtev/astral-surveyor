...
export class Ship {
    ...
    render(renderer, rotation = 0, cameraX = 0, cameraY = 0, activeStars = []) {
        ...
        const coloredSprite = this.sprite.map(row => row.split('').map(char => char === '#' ? shipColor : char));
...
export class ThrusterParticles {
    ...
    render(renderer) {
        ...
        for (const particle of this.particles) {
            ...
            renderer.drawPixel(centerX + particle.x, centerY + particle.y, color, 1);
        }
    }
}
export class StarParticles {
    ...
    spawnParticles(star, deltaTime) {
        ...
        if (starParticles.length >= this.maxParticlesPerStar)
            return;
